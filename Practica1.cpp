//Practica 1. Pimentel Escobar Alondra Valeria
...
std::mt19937 genera(rd());
std::uniform_real_distribution<float> rango(0.0f, 1.0f);

float rojo = rango(genera);
float verde = rango(genera);
float azul = rango(genera);
...
int main()
{
 ...
 //Llamada a las funciones creadas antes del main
	...
	while (!glfwWindowShouldClose(mainWindow))
	{
		...
		double tiempoA= glfwGetTime();
		
	
		if (tiempoA-tiempoD >= 2.0) {
			rojo = rango(genera);
			verde = rango(genera);
		    azul = rango(genera);	
			tiempoD = tiempoA;	
